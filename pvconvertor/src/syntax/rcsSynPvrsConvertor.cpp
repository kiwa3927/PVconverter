...
const std::string rcsSynPvrsConvertor::m_sTmpLayerPrefix = "pvrs_tmp_layer_";
const hvUInt32 MAX_OPTIONS_PER_LINE = 6;

rcsSynPvrsConvertor::rcsSynPvrsConvertor(std::map<hvUInt32, std::pair<hvUInt32, bool> > &blankLinesBefore)
: m_oPVRSStream(&(rcsManager_T::getInstance()->getPVRSOutStream())),
  ...
  m_inProcReturn(false), m_blankLinesBefore(blankLinesBefore)
{
    ...
    else
    {
    	...
    	else
    	{
    		...
    		if(rcsManager_T::getInstance()->isCallFromSetLayerCommand() ||
    				sLayerName.find("pvrs_tmp_layer_") != std::string::npos)
    		{
    			prefix = "trs::setlayer ";
    		}
    		else
...
rcsSynPvrsConvertor::endVisitLayerDefinitionNode(rcsSynLayerDefinitionNode_T *pNode)
{
            ...
            !rcsManager_T::getInstance()->needExpandTmpLayer())
    {
        ...
        m_vTmpLayerValues[m_sTmpLayerName] = static_cast<std::ostringstream *>(m_oPVRSStream)->str();
...
rcsSynPvrsConvertor::convMaxLimitSpec(rcsSynSpecificationNode_T *pNode)
{
	...
	switch (pNode->getSpecificationType())
	{
		...
		{
			sCmd += "drc_tolerance ( distance ";
			break;
		}
		case DRC_TOLERANCE_FACTOR_NAR:
		{
			sCmd += "drc_tolerance ( nar ";
			break;
		}
        case LVS_SHOW_SEED_PROMOTIONS_MAXIMUM:
        {
			sCmd += "lvs_output_device_promotion_max ( ";
			break;
        }
        ...
        {
			sCmd += "erc_max_nets_per_open ( ";
			break;
        }
        case TEXT_PRINT_MAXIMUM:
        {
			sCmd += "print_log ( text_max ";
			break;
        }
        case LAYOUT_BUMP2:
        {
        ...
        {
            sCmd += "drc_max_unattached_label ( ";
        	break;
        }
        case DRC_MAXIMUM_CELL_NAME_LENGTH:
        {
            sCmd += "drc_max_cell_name ( ";
        	break;
        }
		default:
		{
...
{
	(*m_oPVRSStream) << "drc_map_text_layer ( ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    ++iter;
...
rcsSynPvrsConvertor::convPrecSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
        (*m_oPVRSStream) << "precision_db ( ";
    }
    else if (pNode->getSpecificationType() == PRECISION)
    {
        (*m_oPVRSStream) << "precision ( ";
    }
    else
    {
    	return;
...
rcsSynPvrsConvertor::convYesNoSpec(rcsSynSpecificationNode_T *pNode)
{
	...
	switch(pNode->getSpecificationType())
	{
        case DRC_BOOLEAN_NOSNAP45:
            sCmd += "drc_merge_nosnap45 ( ";
            break;
		case DRC_CELL_TEXT:
			sCmd += "drc_process_cell_text ( ";
			break;
		case DRC_EXCLUDE_FALSE_NOTCH:
			sCmd += "drc_output_reduce_false_error ( ";
			break;
		case DRC_INCREMENTAL_CONNECT:
			sCmd += "incremental_connect ( ";
			break;
		case DRC_INCREMENTAL_CONNECT_WARNING:
			sCmd += "incremental_connect_warning ( ";
			break;
		case DRC_ICSTATION:
			sCmd += "gui_priority ( ";
			break;
		case DRC_KEEP_EMPTY:
			sCmd += "drc_output_empty ( ";
			break;
		case ERC_KEEP_EMPTY:
			sCmd += "erc_output_empty ( ";
			break;
		case DRC_MAP_TEXT:
		...
		case VIRTUAL_CONNECT_BOX_COLON:
			sCmd += "virtual_connect ( black_box_colon ";
			break;
		case VIRTUAL_CONNECT_COLON:
			sCmd += "virtual_connect ( colon ";
			break;
		case VIRTUAL_CONNECT_INCREMENTAL:
			sCmd += "virtual_connect ( incremental_connect ";
			break;
		case VIRTUAL_CONNECT_REPORT:
			sCmd += "virtual_connect ( report_warning ";
			break;
		case VIRTUAL_CONNECT_SEMICOLON_AS_COLON:
			sCmd += "virtual_connect ( semicolon ";
			break;
		case LAYOUT_PRESERVE_CASE:
			sCmd += "case_sensitive ( layout_property_net ";
			break;
		case LAYOUT_PRESERVE_NET_CASE:
			sCmd += "case_sensitive ( layout_net ";
			break;
		case LAYOUT_PRESERVE_PROPERTY_CASE:
			sCmd += "case_sensitive ( layout_property ";
			break;
		case LVS_ABORT_ON_ERC_ERROR:
			sCmd += "quit ( erc_error ";
			break;
		case LVS_ABORT_ON_SOFTCHK:
			sCmd += "quit ( uniconnect_check ";
			break;
		case LVS_ABORT_ON_SUPPLY_ERROR:
			sCmd += "quit ( power_ground_error ";
			break;
		case LVS_ALL_CAPACITOR_PINS_SWAPPABLE:
			sCmd += "device_pin_swap ( cap ";
			break;
		case LVS_BUILTIN_DEVICE_PIN_SWAP:
			sCmd += "device_pin_swap ( mos_res ";
			break;
		case LVS_CELL_SUPPLY:
			sCmd += "lvs_subcell_power_ground ( ";
			break;
		case LVS_CHECK_PORT_NAMES:
			sCmd += "lvs_check_top_cell_port ( ";
			break;
		case LVS_DISCARD_PINS_BY_DEVICE:
			sCmd += "lvs_ignore_extra_pin ( ";
			break;
		case LVS_DOWNCASE_DEVICE:
			sCmd += "case_sensitive ( lower_device_case  ";
			break;
		case LVS_EXACT_SUBTYPES:
			sCmd += "lvs_match_accurate_subtype ( ";
			break;
		case LVS_EXECUTE_ERC:
			sCmd += "lvs_run_erc ( ";
			break;
		case LVS_EXPAND_SEED_PROMOTIONS:
			sCmd += "lvs_expand_hcell ( device_promotion ";
			break;
		case LVS_EXPAND_UNBALANCED_CELLS:
			sCmd += "lvs_expand_hcell ( unbalance ";
			break;
		case LVS_FILTER_UNUSED_BIPOLAR:
			sCmd += "device_filter_unused ( bjt ";
			break;
		case LVS_FILTER_UNUSED_CAPACITORS:
			sCmd += "device_filter_unused ( cap ";
			break;
		case LVS_FILTER_UNUSED_DIODES:
			sCmd += "device_filter_unused ( dio ";
			break;
		case LVS_FILTER_UNUSED_MOS:
			sCmd += "device_filter_unused ( mos ";
			break;
		case LVS_FILTER_UNUSED_RESISTORS:
			sCmd += "device_filter_unused ( res ";
			break;
		case LVS_GLOBALS_ARE_PORTS:
			sCmd += "lvs_global_net_as_port ( ";
			break;
		case LVS_IGNORE_PORTS:
			sCmd += "lvs_ignore_port ( top_cell ";
			break;
		case LVS_IGNORE_TRIVIAL_NAMED_PORTS:
            sCmd += "lvs_ignore_port ( sub_cell ";
			break;
		case LVS_INJECT_LOGIC:
			sCmd += "lvs_inject_hierarchy ( ";
			break;
		case LVS_NETLIST_ALL_TEXTED_PINS:
			sCmd += "netlist ( extract_all_texted_pin ";
			break;
		case LVS_NETLIST_ALLOW_INCONSISTENT_MODEL:
			sCmd += "netlist ( extract_inconsistent_model ";
			break;
		case LVS_NETLIST_BOX_CONTENTS:
			sCmd += "netlist ( extract_black_box_content ";
			break;
		case LVS_NETLIST_COMMENT_CODED_SUBSTRATE:
			sCmd += "netlist ( substrate_in_comment ";
			break;
		case LVS_NETLIST_COMMENT_CODED_PROPERTIES:
			sCmd += "netlist ( property_in_comment ";
			break;
		case LVS_NETLIST_UNNAMED_BOX_PINS:
			sCmd += "netlist ( unnamed_black_box_pin ";
			break;
		case LVS_OUT_OF_RANGE_EXCLUDE_ZERO:
			sCmd += "lvs_zero_property_with_tol ( ";
			break;
		case LVS_PRESERVE_BOX_CELLS:
			sCmd += "lvs_keep_black_box ( cell ";
			break;
		case LVS_PRESERVE_BOX_PORTS:
			sCmd += "lvs_keep_black_box ( port ";
			break;
		case LVS_PRESERVE_FLOATING_TOP_NETS:
			sCmd += "lvs_keep_floating_top_net ( ";
			break;
		case LVS_PRESERVE_PARAMETERIZED_CELLS:
			sCmd += "lvs_keep_pcell ( ";
			break;
		case LVS_REPORT_UNITS:
			sCmd += "lvs_report ( unit ";
			break;
		case LVS_REPORT_WARNINGS_HCELL_ONLY:
			sCmd += "lvs_report ( only_warning_hcell ";
			break;
		case LVS_REPORT_WARNINGS_TOP_ONLY:
			sCmd += "lvs_report ( only_warning_top ";
			break;
		case LVS_REVERSE_WL:
			sCmd += "spice ( swap_wl ";
			break;
		case LVS_SHOW_SEED_PROMOTIONS:
			sCmd += "lvs_output_device_promotion ( ";
			break;
		case LVS_SOFT_SUBSTRATE_PINS:
			sCmd += "lvs_ignore_substrate_pin ( ";
			break;
		case LVS_SPICE_ALLOW_DUPLICATE_SUBCIRCUIT_NAMES:
			sCmd += "spice ( permit_duplicate_subcircuit ";
			break;
		case LVS_SPICE_ALLOW_FLOATING_PINS:
			sCmd += "spice ( permit_floating_pin ";
			break;
		case LVS_SPICE_ALLOW_INLINE_PARAMETERS:
			sCmd += "spice ( permit_inline_parameter ";
			break;
		case LVS_SPICE_ALLOW_UNQUOTED_STRINGS:
			sCmd += "spice ( permit_unquoted_string ";
			break;
		case LVS_SPICE_CONDITIONAL_LDD:
			sCmd += "spice ( conditional_comment_ldd ";
			break;
		case LVS_SPICE_CULL_PRIMITIVE_SUBCIRCUITS:
			sCmd += "spice ( ignore_primitive_subcircuit ";
			break;
		case LVS_SPICE_IMPLIED_MOS_AREA:
			sCmd += "spice ( calculate_mos_area ";
			break;
		case LVS_SPICE_OVERRIDE_GLOBALS:
			sCmd += "spice ( rewrite_global_net ";
			break;
		case LVS_SPICE_PREFER_PINS:
			sCmd += "spice ( use_subcell_pin ";
			break;
		case LVS_SPICE_REDEFINE_PARAM:
			sCmd += "spice ( redefine_param_value ";
			break;
		case LVS_SPICE_REPLICATE_DEVICES:
			sCmd += "spice ( split_multipler_device  ";
			break;
		case LVS_SPICE_SLASH_IS_SPACE:
			sCmd += "spice ( treat_slash_as_space ";
			break;
		case LVS_SPICE_STRICT_WL:
			sCmd += "spice ( strict_wl ";
			break;
		case LVS_STRICT_SUBTYPES:
			sCmd += "lvs_match_strict_subtype ( ";
			break;
		case LVS_PRECISE_INTERACTION:
			sCmd += "lvs_precise_overlap ( ";
			break;
		case SOURCE_CASE:
			sCmd += "case_sensitive ( schematic ";
			break;
		case LAYOUT_CASE:
			sCmd += "case_sensitive ( layout ";
			break;
        case LAYOUT_PROCESS_BOX_RECORD:
        	sCmd += "layout_read ( box_record ";
        	break;
        case LAYOUT_PROCESS_NODE_RECORD:
        	sCmd += "layout_read ( node_record ";
        	break;
        case LAYOUT_ALLOW_DUPLICATE_CELL:
        	sCmd += "layout_read ( duplicate_cell ";
        	break;
        case LAYOUT_WINDOW_CLIP:
        	sCmd += "layout_select_window ( clip ";
        	break;
        case LAYOUT_USE_DATABASE_PRECISION:
        	sCmd += "layout_use_db_precision ( ";
        	break;
        case LAYOUT_ERROR_ON_INPUT:
        	sCmd += "quit ( layout_error ";
        	break;
        case SNAP_OFFGRID:
        	sCmd += "snap_offgrid ( ";
        	break;
        case FLATTEN_VERY_SMALL_CELLS:
        	sCmd += "flatten ( very_small_cell ";
        	break;
        case LVS_NL_PIN_LOCATIONS:
            sCmd += "netlist ( extract_pin_location ";
            break;
		default:
			return;
	}
...
{
	std::string sCmd = "schematic_input ( ";
...
rcsSynPvrsConvertor::convLayerListSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
        case TEXT_LAYER:
        	sCmd += "label_layer ( text ";
        	break;
        case PORT_LAYER_POLYGON:
        	sCmd += "label_layer ( port_polygon ";
        	break;
        case PORT_LAYER_TEXT:
        	sCmd += "label_layer ( port ";
        	break;
        case EXCLUDE_ACUTE:
        	sCmd += "check_original_layer ( ignore_acute_edge ";
        	break;
        case EXCLUDE_ANGLED:
        	sCmd += "check_original_layer ( ignore_angled_edge ";
        	break;
        case EXCLUDE_OFFGRID:
        	sCmd += "check_original_layer ( ignore_offgrid_vertex ";
        	break;
        case EXCLUDE_SKEW:
        	sCmd += "check_original_layer ( ignore_skew_edge ";
        	break;
        case LAYOUT_TOP_LAYER:
        	sCmd += "layout_connect_layer ( ";
        	break;
        case LAYOUT_BASE_LAYER:
        	sCmd += "layout_device_layer ( ";
        	break;
        case DRC_ERROR_WAIVE_LAYER:
        	sCmd += "drc_waive_error ( by_layer ";
        	break;
        case LAYOUT_WINDEL_LAYER:
        	sCmd += "layout_exclude_window ( layer ";
        	break;
        case LAYOUT_WINDOW_LAYER:
        	sCmd += "layout_select_window ( layer ";
        	break;
        case DRC_PRINT_AREA:
        	sCmd += "print_log ( area ";
        	break;
        case DRC_PRINT_PERIMETER:
        	sCmd += "print_log ( perimeter ";
        	break;
		default:
			return;
    }
...
rcsSynPvrsConvertor::convDeviceSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (!hasInCompleteBuiltIn) (*m_oPVRSStream) << ")";
    if (!m_isInLayerDefinition)
    {
        (*m_oPVRSStream) << std::endl;
    }
...
rcsSynPvrsConvertor::convCellNameSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
        (*m_oPVRSStream) << "drc_output_cell_name ( ";
    }
    else if (pNode->getSpecificationType() == ERC_CELL_NAME)
    {
        (*m_oPVRSStream) << "erc_output_cell_name ( ";
    }
    else
    {
    	return;
...
rcsSynPvrsConvertor::convRuleSelectSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    switch(pNode->getSpecificationType())
    {
		case DRC_SELECT_CHECK:
			sCmd += "drc_select_rule ( by_name ";
			break;
		case DRC_UNSELECT_CHECK:
			sCmd += "drc_ignore_rule ( by_name ";
			break;
		case DRC_SELECT_CHECK_BY_LAYER:
			sCmd += "drc_select_rule ( by_layer ";
			break;
		case DRC_UNSELECT_CHECK_BY_LAYER:
			sCmd += "drc_ignore_rule ( by_layer ";
			break;
		case ERC_SELECT_CHECK:
			sCmd += "erc_select_rule ( by_name ";
			break;
		case ERC_SELECT_CHECK_ABORT_LVS:
			sCmd += "erc_select_rule ( quit_lvs by_name ";
			break;
		case ERC_UNSELECT_CHECK:
			sCmd += "erc_ignore_rule ( by_name ";
			break;
		case DFM_SELECT_CHECK:
			sCmd += "dfm_select_rule ( by_name ";
			break;
		case DFM_UNSELECT_CHECK:
			sCmd += "dfm_ignore_rule ( by_name ";
			break;
		default:
			return;
    }
...
rcsSynPvrsConvertor::convRuleTextSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
        (*m_oPVRSStream) << "drc_output_rule_text ( ";
    }
    else if (pNode->getSpecificationType() == ERC_CHECK_TEXT)
	{
        (*m_oPVRSStream) << "erc_output_rule_text ( ";
	}
    else
    {
    	return;
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "case_sensitive ( compare ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    ++iter;
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_output_netlist ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_merge ( split_gate ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_short_equivalent_net ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_merge ( user_define ";
...
rcsSynPvrsConvertor::convDepthSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    switch (pNode->getSpecificationType())
    {
		case DRC_MAP_TEXT_DEPTH:
			...
			sCmd += "drc_map_text_level ( ";
			break;
		case LAYOUT_DEPTH:
			sCmd += "layout_read_level ( ";
			break;
		case TEXT_DEPTH:
			sCmd += "label_level ( text ";
			break;
		case PORT_DEPTH:
			sCmd += "label_level ( port ";
			break;
		case VIRTUAL_CONNECT_DEPTH:
			sCmd += "virtual_connect ( level ";
			break;
		default:
			return;
    }
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_filter_option ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_merge_priority ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_property ( init ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_property ( map ";
...
rcsSynPvrsConvertor::convPushDeviceSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    std::string sCmd = "device_push ( ";
    ++iter;
    if (iter != pNode->end())
    {
    	std::list<rcsToken_T>::iterator iter2 = iter;
    	++iter2;
    	if (iter2 == pNode->end())
    	{
            if (iter->eType == SECONDARY_KEYWORD && iter->eKeyType == YES)
            ...
            else if (iter->eType == SECONDARY_KEYWORD && iter->eKeyType == LDE)
            {
            	outputCon2Error(pNode,*iter);
                sCmd += "yes ";
            }
    	...
    	else if ((iter->eType == SECONDARY_KEYWORD && iter->eKeyType == YES)
    			&& (iter2->eType == SECONDARY_KEYWORD && iter2->eKeyType == LDE))
    	{
    		++iter2;
    		if (iter2 == pNode->end())
    		{
    		...
    		else
    		{
    			...
    			&& (iter2->eType == SECONDARY_KEYWORD && iter2->eKeyType == YES))
    	{
    		++iter2;
    		if (iter2 == pNode->end())
    		{
...
rcsSynPvrsConvertor::convLvsSplitGateRatio(rcsSynSpecificationNode_T *pNode)
{
    ...
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "device_merge ( split_gate_ratio ";
...
rcsSynPvrsConvertor::convOutputMagSpec(rcsSynSpecificationNode_T *pNode)
{
    if (pNode->getSpecificationType() == DRC_MAGNIFY_RESULTS)
        (*m_oPVRSStream) << "drc_magnify_result ( db ";
    else if (pNode->getSpecificationType() == DRC_MAGNIFY_DENSITY)
        (*m_oPVRSStream) << "drc_magnify_result ( density ";
    else if (pNode->getSpecificationType() == DRC_MAGNIFY_NAR)
        (*m_oPVRSStream) << "drc_magnify_result ( nar ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "drc_result ( precision ";
...
rcsSynPvrsConvertor::convCellListSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    switch(pNode->getSpecificationType())
    {
		case LAYOUT_PRESERVE_CELL_LIST:
			sCmd += "layout_reserve_cell_group ( ";
			break;
		case DRC_ERROR_WAIVE_CELL:
			sCmd += "drc_waive_error ( by_cell ";
			break;
		case EXCLUDE_CELL:
			sCmd += "ignore_cell ( ";
			break;
		case PUSH_CELL:
			sCmd += "push_cell_lowest ( ";
			break;
		case EXPAND_CELL:
			sCmd += "promote_cell ( ";
			break;
		...
		case FLATTEN_INSIDE_CELL:
			sCmd += "flatten ( in_cell ";
			break;
		case LAYOUT_BASE_CELL:
			sCmd += "layout_device_cell ( ";
			break;
		case LAYOUT_WINDEL_CELL:
			sCmd += "layout_exclude_window ( cell ";
			break;
		case LAYOUT_WINDOW_CELL:
			sCmd += "layout_select_window ( cell ";
			break;
		default:
			return;
    }
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "layout_add_polygon ( ";
...
rcsSynPvrsConvertor::convCellListDefSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (pNode->getSpecificationType() == LAYOUT_CELL_LIST)
        (*m_oPVRSStream) << "layout_cell_group ( ";
    else if (pNode->getSpecificationType() == LVS_CELL_LIST)
        (*m_oPVRSStream) << "lvs_cell_group ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "layout_exception ( db ";
...
rcsSynPvrsConvertor::convExceptionSeveritySpec(rcsSynSpecificationNode_T *pNode)
{
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    ++iter;
    std::string sCmd =  "layout_exception ( severity ";
    if (iter != pNode->end())
    {
...
rcsSynPvrsConvertor::convCoordListSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
    	(*m_oPVRSStream) << "layout_select_window ( region ";
    }
    else if (pNode->getSpecificationType() == LAYOUT_WINDEL)
    {
    	(*m_oPVRSStream) << "layout_exclude_window ( region ";
    }
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "layout_merge_on_input ( ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    ++iter;
...
rcsSynPvrsConvertor::convRenameTextSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (pNode->getSpecificationType() == LAYOUT_IGNORE_TEXT)
        (*m_oPVRSStream) << "layout_remove_text ( ";
    else
        (*m_oPVRSStream) << "layout_rename_text ( ";
            ...
            case SECONDARY_KEYWORD:
            {
                ...
                else if (BY_LAYER == iter->eKeyType)
                {
                       ...
                       && TO == iter->eKeyType)
                    {
                        ...
                        if (!getLayerRefName(++iter, pNode->end(), sLayerName))
                            sLayerName = iter->sValue;
                        outputText("to", sLayerName.c_str(), nOption++);
                    }
                    continue;
                }
...
{
    (*m_oPVRSStream) << "resolution ( layer ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "layout_rename_cell ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_black_box_port ( ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    while(++iter != pNode->end())
    {
        std::string sValue;
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_black_box_port_level ( ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    while(++iter != pNode->end())
    {
...
{
    (*m_oPVRSStream) << "lvs_check_property_radius ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_corresponding_net ( ";
...
rcsSynPvrsConvertor::convNameListSpec(rcsSynSpecificationNode_T *pNode)
{
	switch (pNode->getSpecificationType())
	{
		case VIRTUAL_CONNECT_BOX_NAME:
			(*m_oPVRSStream) << "virtual_connect ( black_box_name ";
			break;
		case VIRTUAL_CONNECT_NAME:
			(*m_oPVRSStream) << "virtual_connect ( name ";
			break;
		case LVS_EXCLUDE_HCELL:
			(*m_oPVRSStream) << "lvs_ignore_hcell ( ";
			break;
		case LVS_GROUND_NAME:
			(*m_oPVRSStream) << "lvs_ground ( ";
			break;
		case LVS_POWER_NAME:
			(*m_oPVRSStream) << "lvs_power ( ";
			break;
		default:
			return;
	}
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_identify_gate ( ";
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_flatten_inside_cell ( ";
    std::list<rcsToken_T>::iterator iter = pNode->begin();
    while (++iter != pNode->end())
    {
        if (iter->eType == SECONDARY_KEYWORD)
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_layout_global_net_group ( ";
...
rcsSynPvrsConvertor::convNonMatchNameSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (pNode->getSpecificationType() == LVS_NON_USER_NAME_INSTANCE)
        (*m_oPVRSStream) << "lvs_non_user_define_name( instance ";
    else if (pNode->getSpecificationType() == LVS_NON_USER_NAME_NET)
        (*m_oPVRSStream) << "lvs_non_user_define_name( net ";
    else if (pNode->getSpecificationType() == LVS_NON_USER_NAME_PORT)
        (*m_oPVRSStream) << "lvs_non_user_define_name( port ";
...
rcsSynPvrsConvertor::convLvsReportOptionSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "lvs_report ( option ";
...
rcsSynPvrsConvertor::convPutsSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (getStringName(iter, pNode->end(), sMessage))
        (*m_oPVRSStream) << "print_log ( compile " << sMessage << " )";
...
{
    std::string sCmd = "promote_cell_text ( ";
...
rcsSynPvrsConvertor::convErrorSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    std::string sValue;
    if (getStringName(iter, pNode->end(), sValue))
        (*m_oPVRSStream) << "quit ( compile_error " << sValue << std::endl;
...
rcsSynPvrsConvertor::convSpiceOptionSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    switch(pNode->getSpecificationType())
    {
        ...
        {
            (*m_oPVRSStream) << "spice ( multiplier_factor_name ";
            for(; iter != pNode->end(); ++iter)
            {
                std::string sValue;
                if (getStringName(iter, pNode->end(), sValue))
...
{
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "spice ( rename_param ";
...
rcsSynPvrsConvertor::convSconnectCheckSpec(rcsSynSpecificationNode_T *pNode)
{
    ...
    hvUInt32 nOption = 0;
    (*m_oPVRSStream) << "uniconnect_check ( ";
...
rcsSynPvrsConvertor::convSoftChkMaxResSpec(rcsSynSpecificationNode_T *pNode)
{
	switch (pNode->getSpecificationType())
	{
		case LVS_SOFTCHK_MAXIMUM_RESULTS:
			(*m_oPVRSStream) << "uniconnect_check_max_result ( ";
			break;
		case VIRTUAL_CONNECT_REPORT_MAXIMUM:
			(*m_oPVRSStream) << "virtual_connect ( report_max ";
			break;
		default:
			return;
	}
...
rcsSynPvrsConvertor::convTvfFuncSpec(rcsSynSpecificationNode_T *pNode)
{
	...
	while (iter != pNode->end())
	{
		...
		rcsTvfConvertor convertor("NULL");
		convertor.execute(script.c_str());
...
rcsSynPvrsConvertor::convOldIsolateNetSpec(rcsSynSpecificationNode_T *pNode)
{
	...
	switch (pNode->getSpecificationType())
	{
		case LVS_ISOLATE_SHORTS_IGNORE_NETS:
			sCmd = "erc_find_shorts_ignore_nets ( ";
			break;
		case LVS_ISOLATE_OPENS_NETS:
			sCmd = "erc_find_open_nets ( ";
			break;
		case LVS_ISOLATE_OPENS_IGNORE_NETS:
			sCmd = "erc_find_open_ignore_nets ( ";
			break;
		default:
			outputCon1Error(pNode);
			return;
                                     ...
                                     std::string &sLayerName)
{
    switch(iter->eType)
    {
        ...
        case STRING_CONSTANTS:
        {
            bool isTmpLayer = strncmp(sLayerName.c_str(), m_sTmpLayerPrefix.c_str(), strlen(m_sTmpLayerPrefix.c_str())) == 0;
                                  ...
                                  std::string &sLayerName)
{
    switch(iter->eType)
    {
        ...
        case STRING_CONSTANTS:
        {
            bool isTmpLayer = strncmp(iter->sValue.c_str(), m_sTmpLayerPrefix.c_str(), strlen(m_sTmpLayerPrefix.c_str())) == 0;
            if (!isTmpLayer && !iter->namescopes.empty())
            {
        ...
        switch(iter->eType)
        {
            ...
            case SECONDARY_KEYWORD:
            {
                ...
                else if(iter->eKeyType == BY)
                {
                    ...
                    if (getLayerRefName(iter, pNode->end(), sLayerName))
                    {
                        outputText("by", sLayerName.c_str(), nOption++);
                        continue;
                    }
                }
                else if(iter->eKeyType == WITH)
                {
                    ...
                    if (getLayerRefName(iter, pNode->end(), sLayerName))
                    {
                        outputText("with", sLayerName.c_str(), nOption++);
                        continue;
                    }
                }
            }
        ...
        if (SECONDARY_KEYWORD == iter->eType)
        {
            switch(iter->eKeyType)
            {
                ...
                case WITH_LAYER:
                {
                    ...
                    if (getLayerRefName(iter, pNode->end(), sLayerName))
                    {
                       outputText("with_layer", sLayerName.c_str(), nOption++);
                       continue;
                    }
                    break;
                }
        ...
        if (SECONDARY_KEYWORD == iter->eType)
        {
            switch(iter->eKeyType)
            {
                ...
                case BY_LAYER:
                {
                    ...
                    if (!getLayerRefName(++iter, pNode->end(), sLayerName))
                        sLayerName = iter->sValue;
                    outputText("shield_layer", sLayerName.c_str(), nOption++);
                    continue;
                }
                case CONNECTED:
                {
            ...
            switch(iter->eKeyType)
            {
                ...
                case MAXIMUM:
                {
                    ...
                    if (iter != pNode->end() && iter->eType == SECONDARY_KEYWORD &&
                       iter->eKeyType == ALL)
                    {
                       outputText("max_result", "all", nOption++);
                        ++iter;
                        continue;
                    }
                    else
                    {
                        std::string sNumExp;
                        if (getNumberExp(iter, pNode->end(), sNumExp))
                            outputText("max_result", sNumExp.c_str(), nOption++);
                        continue;
                    }
                }
                default:
...
rcsSynPvrsConvertor::convFillRectanglesCmd(rcsSynLayerOperationNode_T *pNode)
{
    ...
    {
        outputText("inside_of_layer", sInsideLayer.c_str(), nOption++);
    }
...
void rcsSynPvrsConvertor::convLayoutCloneRotatedPlacements(rcsSynSpecificationNode_T *pNode)
{
    ...
    {
        (*m_oPVRSStream) << "layout_clone ( rotated_placement " << it->sValue << " )\n";
    }
}
...
void rcsSynPvrsConvertor::convLayoutCloneTransformedPlacements(rcsSynSpecificationNode_T *pNode)
{
    ...
    if (it != pNode->end() && (it->eKeyType == NO || it->eKeyType == YES))
    {
        (*m_oPVRSStream) << "layout_clone ( transformed_placement " << it->sValue << " )\n";
    }
}
